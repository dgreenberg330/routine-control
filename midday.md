...
barclayscenter.com: https://www.barclayscenter.com/events/category/concerts is server-rendered (curl --compressed, NO browser User-Agent: a Chrome UA gets 406) (2026-09-26).
...
guggenheim.org: https://www.guggenheim.org/exhibitions shell is JS, but exhibition data is embedded as JSON in the HTML (curl --compressed). Find each "dates":{ block; inside it "start"/"end" are {"day","month"(NAME string),"year"} but key order varies and a "label" key may sit between them, so match start and end separately; title is the first "title":"..." after the block (within ~3KB, may contain <br>) (2026-09-26).
...
msg.com: plain fetch has no listings and direct api.msg.com call is 401. Render https://www.msg.com/calendar in Playwright (proxy), press Escape to close opt-in modal, click "Load more" (force) ~5x, capture api.msg.com/v3.0/events?page=N responses (results[].name, category[].value=='music', show_times[0].date_time ms, msg_edp_url has venue slug; skip chicago-theatre). date_time is UTC ms: subtract 4h (EDT) / 5h (EST) or evening shows land a day late. On 2026-09-26 the "Load more" click stopped after 2 pages (listings only reached ~Oct 12); try a different selector next run. Tracker/font domains are proxy-denied, harmless (2026-09-26).
